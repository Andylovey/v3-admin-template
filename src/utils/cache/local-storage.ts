...
import { LocaleLang } from "@/constants/local-storage-key"
...
//#region 语言
export const getCachedLocale = (): LocaleLang => {
  return localStorage.getItem(CacheKey.LOCALE) as LocaleLang
}
export const setCachedLocale = (locale: LocaleLang) => {
...
export const getCachedMenus = () => {
  const json = localStorage.getItem(CacheKey.MENUS)
  return JSON.parse(json ?? "[]") as string[]
}
export const setCachedMenus = (menus: any) => {
  localStorage.setItem(CacheKey.MENUS,JSON.stringify(menus))
}
//#endregion